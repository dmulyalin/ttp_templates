"""
Normalize Arista EOS VLAN configuration parsed by TTP.

Transforms ``show running-config section vlan`` stanzas into a flat list of
VLAN dictionaries suitable for getter-style consumption.

Used by:
- ttp_templates/platform/arista_eos_show_running_config_section_vlan.txt
"""
...
def transform_vlans_config(payload: list) -> List[Dict[str, Any]]:
    """
    Convert parsed Arista VLAN stanzas into normalized VLAN records.
