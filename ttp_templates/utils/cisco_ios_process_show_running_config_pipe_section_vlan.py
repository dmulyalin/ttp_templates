...
Transforms VLAN and interface sections from IOS running configuration into a
flat list of VLAN dictionaries suitable for getter-style consumption.
...
def transform_vlans_config(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for interface in payload.get("interfaces", []):
        ...
        for prefix in ("vlan", "bvi"):
            suffix = name_lower.removeprefix(prefix)
            if suffix != name_lower and suffix.isdigit():
                untagged_vid = int(suffix)
