...
def _normalize_route_target(value: Any) -> str:
    """Return route-target community without NX-OS suffixes such as ``evpn``."""
    if isinstance(value, dict):
        value = value.get("rt")
    return str(value).strip().strip('"').split()[0]


def _as_list(value: Any) -> List[str]:
    """Return a deduplicated list of non-empty route-target strings."""
    if value is None:
        return []
    values = value if isinstance(value, list) else [value]
    result: List[str] = []
    for item in values:
        item = _normalize_route_target(item)
...
def _collect_values(vrf: Dict[str, Any], key: str) -> List[str]:
    """Collect route-targets from VRF root and address-family sections."""
...
def _first_value(vrf: Dict[str, Any], key: str) -> Any:
    ...
    values = []
    if vrf.get(key):
        values.append(vrf[key])
    address_families = vrf.get("address_families")
    if isinstance(address_families, dict):
        for afi in address_families.values():
            if isinstance(afi, dict) and afi.get(key):
                values.append(afi[key])
    return values[0] if values else None


def transform_vrfs_config(payload: list) -> List[Dict[str, Any]]:
    """
    Convert parsed Cisco NX-OS VRF configuration into normalized VRF records.
