...
def test_N2G_ospf_lsdb_Cisco_IOSXR():
    with open(
        "./mock_data/cisco_xr_show_ip_ospf_database_router_external_summary_router-1.txt",
        "r",
    ) as f:
        data1 = f.read()
    with open(
        "./mock_data/cisco_xr_show_ip_ospf_database_router_external_summary_router-2.txt",
        "r",
    ) as f:
        data2 = f.read()
...
def test_N2G_ospf_lsdb_Cisco_IOS():
    with open(
        "./mock_data/cisco_ios_show_ip_ospf_database_router_external_summary_IOL4_ABR.txt",
        "r",
    ) as f:
        data = f.read()
...
def test_N2G_isis_lsdb_juniper():
    with open(
        "./mock_data/juniper_junos_show_isis_database_verbose_pipe_no_more.txt", "r"
    ) as f:
        data = f.read()
                        ...
                        "R1-X1": [
                            {
                                ...
                                "networks": [
                                    {"metric": "0", "network": "10.123.123.31/32"},
                                    {"metric": "0", "network": "10.123.123.41/32"},
                                    {
                                        "metric": "20",
...
def test_N2G_cli_l2_data_juniper():
    ...
    parser = ttp(
        ...
        template=template,
        vars={
            "IfsNormalize": all_vars["short_interface_names"],
            "physical_ports": all_vars["physical_ports"],
        },
    )
    parser.parse()
    res = parser.result()
    ...
    assert res == [
        {
            "switch-1.net.acme1.lab": {
                ...
                "interfaces_id": [
                    ...
                    {
                        "description": "CID-BN-P-env4207-env4208-0004 " "| BACKHAUL",
                        "id": "612",
            ...
            "ceos1": {
                "interfaces": {
                    "Eth1": {
                        ...
                        "port_description": "Configured by " "NETCONF",
                    },
                    "Lo1": {
                        "ip_addresses": [
                            {"ip": "1.1.1.1", "netmask": "24", "network": "1.1.1.0/24"}
                        ]
                    },
                    "Lo1000": {},
                    "Lo2": {
                        "ip_addresses": [
                            {"ip": "2.2.2.2", "netmask": "24", "network": "2.2.2.0/24"}
                        ],
                        "port_description": "Lopback2 for Customer " "27123",
                    },
                    "Lo3": {
                        "ip_addresses": [
                            {"ip": "1.2.3.4", "netmask": "24", "network": "1.2.3.0/24"}
                        ],
                        "port_description": "Customer #56924 " "service",
                    },
                }
            }
        }
