...
def postprocess(data):
    ...
    for i in data["interfaces"]:
        ...
        if "irb" in i["name"]:
            i["interface_type"] = "bridge"
            i["switching"] = {
                "dot1q_mode": "access",
                "vlans": [int(i["name"].split(".")[1])]
            }
        elif any(
        ...
        if "." in i["name"] and not any(k in i["name"] for k in ["lo0", "vlan", "irb", "gr-"]):
            i["parent"] = i["name"].split(".")[0]

        # form list of vlans
        if i.get("switching", {}).get("vlans", []):
            vlans = []
            for vlan in i.get("switching", {}).get("vlans", []):
                if isinstance(vlan, dict):
                    vlans.append(vlan["vlan"])
                else:
                    vlans.append(vlan)
            i["switching"]["vlans"] = vlans 	
			
        # calculate dot1q mode
        switching = i.get("switching", {})
        if switching.get("dot1q_mode") == "trunk":
            if "all" in switching.get("vlans", []):
                i["switching"]["dot1q_mode"] = "tagged-all"
            else:
                i["switching"]["dot1q_mode"] = "tagged"
        elif switching.get("dot1q_mode") == "access":
            i["switching"]["dot1q_mode"] = "access"
...
set interfaces {{ name }} native-vlan-id {{ dot1q_native_vlan }}					
</group>
...
## VSTP configuration
## ------------------------------------------------------------------------------------------
<group name="vstp**.vlans**.{{ name }}**.{{ interface }}**" method="table">
set protocols vstp vlan {{ name }} interface {{ interface | let("mode", "edge") }} edge
set protocols vstp vlan {{ name }} interface {{ interface | let("no_root_port", True) }} no-root-port
</group>
