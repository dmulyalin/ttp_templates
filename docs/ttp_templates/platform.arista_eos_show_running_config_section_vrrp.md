Reference path:
```
ttp://platform/arista_eos_show_running_config_section_vrrp.txt
```
...
interface {{ interface | _start_ }}
   ...
   vrrp {{ group | DIGIT | to_int }} ip {{ virtual_address | let("address_family", "ipv4") }}
   vrrp {{ group | DIGIT | to_int }} {{ address_family | re("ipv4") }} {{ virtual_address }}
   vrrp {{ group | DIGIT | to_int }} {{ address_family | re("ipv4") }} version {{ version | DIGIT | to_int }}
   ...
   vrrp {{ group | DIGIT | to_int }} {{ address_family | re("ipv6") }} {{ virtual_address }}
   vrrp {{ group | DIGIT | to_int }} priority {{ priority | DIGIT | to_int }}
   vrrp {{ group | DIGIT | to_int }} priority-level {{ priority | DIGIT | to_int }}
   </group>
