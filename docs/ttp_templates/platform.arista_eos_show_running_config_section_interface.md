...
interface {{ name | _start_ }}
   ...
   ip address {{ ip | IP }}/{{ mask }} {{ ip_address_role | set("") }}
   ip address {{ ip | IP }}/{{ mask | let("ip_address_role", "secondary") }} secondary
   ipv4 address {{ ip | IP }}/{{ mask }} {{ ip_address_role | set("") }}
   ipv4 address {{ ip | IP }}/{{ mask | let("ip_address_role", "secondary") }} secondary
   </group>

   <group name="ipv6_addresses*" method="table">
   ipv6 address {{ ip | IPV6 }}/{{ mask | let("ip_address_role", "") }}
   </group>
